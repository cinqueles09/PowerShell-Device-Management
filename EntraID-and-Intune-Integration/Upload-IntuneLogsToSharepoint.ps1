...
﻿Param (  
    ...
    $SharePoint_Path = "https://ywcff.sharepoint.com/sites/LOGS",  # sharepoint main path
    $SharePoint_ExportFolder = "/",  # folder where to upload file
    $File_Path = "C:\Windows\Temp\Logs_$env:computername.zip" # path of the file to upload
)  
...
#Recolecta de logs Intune
mdmdiagnosticstool.exe -area "DeviceEnrollment;DeviceProvisioning;Autopilot" -zip $File_Path
...
$DriveID = $Result.value| Where-Object {$_.webURL -match $SharePoint_Path } | Select-Object id -ExpandProperty id  
...
del C:\Windows\Temp\Logs_$env:computername.zip
