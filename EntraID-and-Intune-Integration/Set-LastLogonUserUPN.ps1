...
.NOTES
NAME: Get-AuthToken
Uses the MSAL.PS module from PowerShellGallery https://www.powershellgallery.com/packages/MSAL.PS/4.37.0.0
Uses MSGraph module instead of AzureAD (marked for deprecation)
Other references: https://tech.nicolonsky.ch/explaining-microsoft-graph-access-token-acquisition/
#>
...
(
    ...
    $userUpn = $User
    # Capture $tenant as a parameter passed along, not as an attribute of the UPN
    Write-Host "Checking for MSGraph module..."
    $MSGraphModule = Get-Module -Name "MSGraph" -ListAvailable

    if ($MSGraphModule -eq $null) {

        Write-Host "MSGraph PowerShell module not found"
        write-host
        write-host "MSGraph Powershell module not installed..." -f Red
        write-host "Install by running 'Install-Module MSGraph' from an elevated PowerShell prompt" -f Yellow
    ...
    if ($MSALModule -eq $null) {
        ...
        write-host "Install by running 'Install-Module MSAL.PS' from an elevated PowerShell prompt" -f Yellow
        write-host "Script can't continue..." -f Red
        write-host
        exit
    }
    ...
    try {
         ...
         if($Password -eq $null -or $Password -eq ""){
            write-host "Password file was null, resetting authResult..."
            ...
            if(test-path "$Password"){
                ...
                Write-Host "Path to Password file " $Password " does not exist, please specify a valid path..." -ForegroundColor Red
